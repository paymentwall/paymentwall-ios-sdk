...
///
/// **Cancellation scope follows ownership:** each owner creates its own transport, and
/// `-cancelAllInFlightRequests` touches only that owner's requests. A payment option implementing
/// `PWPaymentOptionProtocol` is not handed the SDK's HTTP client, so an option that needs to make a
/// request creates one of these and owns it.
...
/// Cancels everything this transport currently has in flight. Requests owned by another transport
/// are untouched.
